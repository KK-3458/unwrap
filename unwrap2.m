...
%         for j=1:400
%             for k=1:100
%                 jj(:)=unwrap(reshape(wrapedphase(i,j,:),[100,1]));
%                 unwrapedphase(i,j,:)=jj;
%             end
%         end
%     end
...
for k=1:100
    max_wrapedphase(k)=max(reshape(wrapedphase2(:,:,k),[360*400,1]));
    min_wrapedphase(k)=min(reshape(wrapedphase2(:,:,k),[360*400,1]));
end
for k=1:100
    
    for i=1:360
        for j=1:400
            normalized_wrapedphase2(i,j,k)=(wrapedphase2(i,j,k)-min_wrapedphase(k))/(max_wrapedphase(k)-min_wrapedphase(k));
        end
    end
end
...
myobj= VideoWriter('myvideo2.avi');
myobj.FrameRate = 5;
open(myobj)
for k=1:100
    writeVideo(myobj, normalized_wrapedphase2(:,:,k));
end
close(myobj)